...
const AvatarSettings: React.FC = () => {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("socioId"); // Limpiar socioId por si acaso
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_WEB}/admin/deletion-requests`,
        {
          headers: {
  ...
  const handleApproveFromNotification = async (
    ...
    try {
      const token = localStorage.getItem("authToken");
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_WEB}/admin/deletion-requests/${requestId}/approve`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleRejectFromNotification = async (
    ...
    try {
      const token = localStorage.getItem("authToken");
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_WEB}/admin/deletion-requests/${requestId}/reject`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
