...
export const aprobarSocio = async (id: number): Promise<void> => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (data.message && data.message.includes("Duplicate entry") && data.message.includes("email_unique")) {
...
export const deleteSocio = async (id: number): Promise<void> => {
  ...
  try {
    const response = await fetch(`${API_URL}/admin/socio/${id}/delete`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
    ...
    if (error instanceof Error) {
      throw new Error(`Error al eliminar el socio: ${error.message}`);
    }
