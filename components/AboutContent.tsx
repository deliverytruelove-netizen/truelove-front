...
export default function Component() {
  ...
  return (
    <div className="flex flex-col bg-white min-h-screen justify-center items-center text-center p-4 md:p-8">
      <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold mb-4 text-gray-800">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-6xl w-full">
        {cards.map((card, index) => (
          <motion.div
            ...
            custom={index}
          >
            <h2 className="font-bold text-xl sm:text-2xl mb-6 text-gray-800 group-hover:text-[#D9043D] transition-colors duration-300">
              {card.title}
            </h2>
            ...
            <p className="font-serif text-sm sm:text-base text-gray-600 group-hover:text-gray-800 transition-colors duration-300">
              {card.description}
            </p>
          </motion.div>
        ))}
      ...
      <button className="mt-12 px-8 py-4 w-full sm:w-auto rounded-full bg-gradient-to-br bg-[#D9043D] shadow-lg shadow-red-500/50 hover:bg-slate-100 text-white font-bold hover:text-red-600">
        Empezar Ahora
      </button>
    </div>
  );
